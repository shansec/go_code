...
func (w withTrimMiddleware) Sum(ctx context.Context, a, b int) (ret int, err error) {
	ret, err = w.next.Sum(ctx, a, b)
	return
}

func (w withTrimMiddleware) Concat(ctx context.Context, a, b string) (res string, err error) {
	// 先调用trim服务，去除字符串中可能存在的空格
	respA, err := w.trimService(ctx, TrimRequest{s: a}) // 请求trim服务处理a
	if err != nil {
		return "", err
	}
	respB, err := w.trimService(ctx, TrimRequest{s: b}) // 请求trim服务处理b
	if err != nil {
		return "", err
	}
